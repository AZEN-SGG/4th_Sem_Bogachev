...
class pattern
{
	...
	public:
		char * get_word () { return word.get(); }
		type_pattern * get_spec () { return spec.get(); }
		borders * get_span () { return span.get(); }
		...
		int with_out (const char *str, int i_word, int i_bord)
		{
			int len = 0;
		...
		int with_underscore (const char *str)
		{
			int len_bord = 0;
			int len = 0;
			for (int i = 0 ; word[i] != '\0' ; ++i, ++len)
			{
				...
				switch (spec[i])
				{
					case type_pattern::one:
						...
						break;
					case type_pattern::in:
						res = is_in_off(str, len, len_bord);
						len_bord++;
						len += 4;
						break;
					case type_pattern::out:
						res = is_out_off(str, len, len_bord);
						len_bord++;
						len += 5;
						break;
					default:
	...
	private:
		...
		{
			return (str[str_i] == '[' &&
				str[str_i + 1] == span[bord_i].start &&
				str[str_i + 2] == '-' &&
				str[str_i + 3] == span[bord_i].end &&
				str[str_i + 4] == ']');
		}
		...
		int is_out_off (const char *str, int str_i, int bord_i)
		{
			return (str[str_i] == '[' &&
				str[str_i + 1] == '^' &&
				str[str_i + 2] == span[bord_i].start &&
				str[str_i + 3] == '-' &&
				str[str_i + 4] == span[bord_i].end &&
				str[str_i + 5] == ']');
		}
